...
void pagSequenceTest() {
  ...
  EXPECT_TRUE(Baseline::Compare(snapshot, "PAGSequenceTest/pagSequenceTest.png"));
}
...
PAG_TEST_F(PAGSequenceTest, BitmapSequenceReader) {
  ...
  pagPlayer->flush();
  auto snapshot = MakeSnapshot(pagSurface);
  EXPECT_TRUE(Baseline::Compare(snapshot, "PAGSequenceTest/BitmapSequenceReader.png"));
}
...
PAG_TEST_F(PAGSequenceTest, VideoSequenceAsMask) {
  ...
  pagPlayer->flush();
  auto snapshot = MakeSnapshot(pagSurface);
  EXPECT_TRUE(Baseline::Compare(snapshot, "PAGSequenceTest/VideoSequenceAsMask.png"));
}
}  // namespace pag
